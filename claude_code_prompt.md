# Claude Code Prompt — Pet Ecosystem MVP Web App

> **How to use this:** Open Claude Code in an empty project folder and paste everything inside the `=== PROMPT START ===` / `=== PROMPT END ===` block as your first message. Fill in the four bracketed placeholders first (brand name, tagline, launch city, primary language). Everything else is ready to run. Keep this file as the project's living spec.
>
> Before pasting, optionally drop your `pet_ecosystem_report.md` into the folder — the prompt tells Claude Code to use it to seed the chatbot's knowledge base.
...
- **Why Next.js over a plain React SPA:** it satisfies every constraint at once — it *is* React, gives you server-side API routes (so the Anthropic key and auth stay server-side), renders server-side for the SEO that your content hub will need, and deploys unchanged from localhost to any Node host or container platform. A Vite SPA would force you to stand up a separate backend anyway; the prompt notes that fallback if you ever want a hard frontend/backend split.
- **Chatbot grounding:** the prompt starts with the simplest workable approach (curated knowledge injected into the system prompt) because your knowledge base is small at MVP. It deliberately puts retrieval behind one function so you can upgrade to embeddings/RAG later — a natural fit given your background — without touching the chat route.
- **Model choice:** `claude-haiku-4-5` keeps a public-facing widget cheap and fast; flipping the one config constant to `claude-sonnet-4-6` buys higher answer quality if you want it.
- **Two things to decide before pasting:** (1) the brand/city/language placeholders, and (2) whether you're fine with Next.js as the stack. If yes, paste as-is.
- This MVP maps to **Phase 1** of the ecosystem report (identity + knowledge + assistant). Services booking, social, and the rest are deliberately excluded so you ship something real first.
